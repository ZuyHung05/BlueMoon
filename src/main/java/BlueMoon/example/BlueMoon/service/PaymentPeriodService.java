...
public class PaymentPeriodService {
    ...
    public PaymentPeriodResponse createPaymentPeriod(PaymentPeriodRequest request) {
        ...
        java.util.Set<Long> validIds = householdRepository.findAll().stream()
                .map(BlueMoon.example.BlueMoon.entity.HouseholdEntity::getHouseholdId).collect(Collectors.toSet());
        return mapToResponse(saved, validIds);
    }

    @Transactional(readOnly = true)
    public List<PaymentPeriodResponse> getAllPaymentPeriods() {
        return getAllPaymentPeriods(null, null);
    }

    @Transactional(readOnly = true)
    public List<PaymentPeriodResponse> getAllPaymentPeriods(LocalDate startDate, LocalDate endDate) {
        java.util.Set<Long> validIds = householdRepository.findAll().stream()
                .map(BlueMoon.example.BlueMoon.entity.HouseholdEntity::getHouseholdId).collect(Collectors.toSet());

        List<PaymentPeriodEntity> periods;
        // Logic: If filter params exist, use them. If one is missing, default to wide
        // range.
        if (startDate != null || endDate != null) {
            ...
            periods = paymentPeriodRepository.findByStartDateBetween(start, end);
        } else {
            periods = paymentPeriodRepository.findAll();
        }
    ...
    private PaymentPeriodResponse mapToResponse(PaymentPeriodEntity entity, java.util.Set<Long> validHouseholdIds) {
        ...
        if (Boolean.TRUE.equals(entity.getIsMandatory())) {
            if (entity.getFees() != null && !entity.getFees().isEmpty()) {
                totalHouseholds = entity.getFees().stream()
                        .filter(f -> f.getHousehold() != null
                                && validHouseholdIds.contains(f.getHousehold().getHouseholdId()))
        ...
        List<PayEntity> validPayments = (entity.getPays() != null)
                ? entity.getPays().stream()
                        .filter(p -> p.getPayDate() != null && p.getAmount().compareTo(BigDecimal.ZERO) > 0
                                && p.getHousehold() != null)
    ...
    public List<BlueMoon.example.BlueMoon.dto.response.PaymentPeriodDetailResponse> getPaymentPeriodDetails(Long id) {
        ...
        return households.stream().map(hh -> {
            ...
            String status = (pay != null && pay.getPayDate() != null && pay.getAmount().compareTo(BigDecimal.ZERO) > 0)
                    ? "Paid"
                    : "Unpaid";
    ...
    public void addPayment(Long paymentPeriodId, BlueMoon.example.BlueMoon.dto.request.PaymentRequest request) {
        ...
        BlueMoon.example.BlueMoon.entity.HouseholdEntity household = householdRepository
                .findById(request.getHouseholdId())
                .orElseThrow(() -> new AppException("Household not found"));

        PayEntity pay = new PayEntity();

        BlueMoon.example.BlueMoon.serializable.PayId payId = new BlueMoon.example.BlueMoon.serializable.PayId(
                request.getHouseholdId(), paymentPeriodId);
        pay.setId(payId);
    ...
    public String debugPaymentPeriod(Long id) {
        ...
        for (PayEntity p : period.getPays()) {
            sb.append("Payment: Household=")
                    .append(p.getHousehold() != null ? p.getHousehold().getHouseholdId() : "NULL");
            ...
            try {
                if (period.getFees() != null) {
                    required = period.getFees().stream()
                            .filter(f -> f.getHousehold() != null && p.getHousehold() != null
                                    && f.getHousehold().getHouseholdId().equals(p.getHousehold().getHouseholdId()))
                            .map(f -> {
                                try {
                                    BigDecimal unit = (f.getUnit() != null && !f.getUnit().isEmpty())
                                            ? new BigDecimal(f.getUnit())
                                            : BigDecimal.ZERO;
            ...
            boolean isValidHousehold = p.getHousehold() != null
                    && validHouseholdIds.contains(p.getHousehold().getHouseholdId());
    ...
    @Transactional
    public void importPaymentData(MultipartFile file, String description, LocalDate startDate, LocalDate endDate,
            Boolean isMandatory, Long paymentPeriodId) {
        try (InputStream is = file.getInputStream();
            ...
            Map<Long, BlueMoon.example.BlueMoon.entity.HouseholdEntity> roomToHousehold = householdRepository.findAll()
                    .stream()
            ...
            if (paymentPeriodId != null) {
                ...
                period = paymentPeriodRepository.findById(paymentPeriodId)
                        .orElseThrow(
                                () -> new RuntimeException("Payment Period ID " + paymentPeriodId + " not found."));
            } else {
    ...
    private void createFeeIfPresent(PaymentPeriodEntity period,
            BlueMoon.example.BlueMoon.entity.HouseholdEntity household, String name, Double val, Double coeff,
            String unit) {
    ...
    private void saveFee(PaymentPeriodEntity period, BlueMoon.example.BlueMoon.entity.HouseholdEntity household,
            String name, BigDecimal quantity, BigDecimal unitPrice, String unit) {
