...
const generateParkingData = (floor) => {
    const bikeSlots = [];
    const carSlots = [];
    ...
    for (let row = 1; row <= 9; row++) {
        for (let col = 1; col <= 7; col++) {
            ...
            bikeSlots.push({
                ...
                col: col,
                zone: zoneLetter,
                status: Math.random() > 0.7 ? (Math.random() > 0.5 ? 'maintenance' : 'empty') : 'registered',
                type: 'bike'
            });
        }
    }
    ...
    for (let row = 1; row <= 6; row++) {
        for (let col = 1; col <= 5; col++) {
            ...
            carSlots.push({
                ...
                col: col,
                zone: zoneLetter,
                status: Math.random() > 0.6 ? (Math.random() > 0.5 ? 'maintenance' : 'empty') : 'registered',
                type: 'car'
            });
        }
    }
...
const ParkingSlot = ({ slot, size = 'small' }) => {
    ...
    return (
        <Tooltip title={`${slot.slotId} - ${slot.status === 'empty' ? 'Trống' : slot.status === 'registered' ? 'Đã đăng ký' : 'Bảo trì'}`}>
            <Box
                sx={{
                    width: width,
                    height: height,
...
const ParkingMap = ({ onBack }) => {
    ...
    const floorData = FLOORS_DATA[currentFloor + 1];
